...
class CoreController extends Controller
{
    ...
    public function dyList($cat, $page)
    {
        ...
        $rules = [
            ...
            'pf' => ['.list.g-clear .s2', 'text', ''],
            'js' => ['.list.g-clear .hint', 'text', ''],
            'year' => ['.list.g-clear .hint', 'text'],
    ...
    public function dsjList($cat, $page)
    {
        ...
        $url = $this->domin . '/dianshi/list?rank=rankhot&cat=' . $cat . '&area=11&act=all&year=all&pageno=' . $page;
        $rules = [
            'title' => ['.list.g-clear .s1', 'text', ''],
            'url' => ['.list.g-clear a.js-tongjic', 'href', '', function ($content) {
                return base64_encode($this->domin . $content);
    ...
    public function getSearch($key){
        ...
        // $rules = [
        //     'title'=>['.b-mainpic_new a','title',''],
        //     'url'=>['.b-mainpic_new a','href','',function($content){
        //          $url = str_replace('http://','https://',$content);
        //          return base64_encode($url);
        //     }],
        //     'img'=>['.b-mainpic_new img','src',''],
        //     'type'=>['.cont .playtype','text',''],
        //     'desc'=>['.js-b-fulldesc','data-full']
        // ];
         $rules = [
            'title'=>['.title a','text',''],
            'url'=>['.title a','href','',function($content){
                 $url = str_replace('http://','https://',$content);
                 return base64_encode($url);
            }],
            'img'=>['.m-mainpic .g-playicon img','src',''],
            'type'=>['.cont .playtype','text',''],
            'desc'=>['.m-description p','text','']
        ];
         $data = $this->ql->get($url)->rules($rules)->query()->getData();
