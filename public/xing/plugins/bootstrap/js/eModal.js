...
; (function (define) {
    define(['jquery'], function ($) {
        ...
        var label = {yes:'是',no:'否',close:'关闭',ok:'确定',cancel:'取消',submit:'提交'};
        //var label = { '确定': '取消', "正确": '错误', '是': '否' };
        var lastParams = {};
        ...
        var defaultSettings = {
            ...
            confirmLabel: {yes:'是',no:'否',close:'关闭',ok:'确定',cancel:'取消',submit:'提交'},
            size: empty,
        ...
        function _getFooter(buttons) {
            ...
            if (buttons) {
                for (var i = 0, k = buttons.length; i < k; i++) {
                    ...
                    for (var ref in btnOp) {
                        if (btnOp.hasOwnProperty(ref)) {
                            switch (ref) {
                                case 'close':
                                    //add close event
                                    if (btnOp[ref]) {
                                ...
                                case eventClick:
                                    ...
                                    break;
                                case 'text':
                                    //console.log(ref+' '+btnOp[ref]);
                                    var $index = btnOp.key;
                                    console.log(ref);
                                    btn.html(label[$index]);
                                    //btn.html(btnOp[ref]);
                                    break;
                                default:
                                    //all other possible HTML attributes to button element
                                    btn.attr(ref, btnOp[ref]);
                            }
                        }
                    }
                ...
                messageFotter.append('<button class="x btn btn-primary" data-dismiss=modal type=button>关闭</button>');
            }
         ...
         */
        function _getModalInstance(skipEventChageIfExists,static) {
            ...
            if (!$modal) {
                ...
                $modal = createModalElement(static);
            }
            //console.log(params);
            ...
            function createModalElement(static) {
                /// <summary></summary>
                /// <returns type="jQuery object"></returns>
                var static_content = static===true ? 'data-backdrop="static"' :'';
                
                return $('<div class="modal fade" tabindex="-1" '+static_content+'><style>.modal-xl{width:96%;}</style>'
                + '<div class=modal-dialog>'
                + '<div class=modal-content>'
                + ' <div class=modal-header><button type=button class="x close" data-dismiss=modal><span aria-hidden=true>&times;</span><span class=sr-only>关闭</span></button><h4 class=modal-title></h4></div>'
                + '</div>'
                + '</div>'
                + '</div>')
        ...
        function _setup(params, title) {
            ...
            // Lazy loading
            var $ref = _getModalInstance(false,params.static);
        ...
        function confirm(data, title) {
           ...
           // console.log(label);
            return alert({
                ...
                buttons: [
                  ...
                  //  { close: true, click: click, text: label[data.label] ? data.label : defaultSettings.confirmLabel }
                     { close: true, click: click,key:'cancel', text: label.cancle, style: keyDanger },
                    { close: true, click: click, key:'ok',text: label.ok }
                ],
                deferred: dfd,
            ...
            function click(ev) {
               ...
               //console.log($(ev.currentTarget).attr("key"));
                return key=='ok' ? dfd.resolve() : dfd.reject();
        ...
        function prompt(data, title) {
            ...
            var buttons = _getFooter([
                { close: true, type: 'reset',key:'cancel',text: label.cancel, style: keyDanger },
                { close: false, type: eventSubmit, key:'ok',text: label.submit }
