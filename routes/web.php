...
Route::get('login.html','Index\IndexController@login')->middleware('CcDefense');//会员登录
Route::get('logout.html','Index\IndexController@logout')->middleware('CcDefense');//会员登录
Route::post('checkLogin','Index\IndexController@checkLogin')->middleware('CcDefense');//会员登录
Route::post('/action/register','Index\actionController@register')->middleware('CcDefense');//会员注册
Route::get('ucenter.html','Index\IndexController@ucenter')->middleware('CcDefense','CheckUserLogin');//会员中心
Route::get('userinfo.html','Index\IndexController@userinfo')->middleware('CcDefense','CheckUserLogin');//会员中心
Route::post('/action/userinfo','Index\actionController@userinfo')->middleware('CcDefense','CheckUserLogin');//会员修改资料
Route::get('chongzhi.html','Index\IndexController@chongzhi')->middleware('CcDefense','CheckUserLogin');//显示充值界面
Route::post('/action/chongzhi','Index\actionController@chongzhi')->middleware('CcDefense','CheckUserLogin');//执行充值
...
Route::get((empty(config('webset.webdir')) ? 'admin' : config('webset.webdir')) . '/sfset', 'Admin\IndexController@sfSet')->middleware('CheckLogin');//收费设置界面
Route::post('action/sfset', 'Admin\actionController@sfSet')->middleware('CheckLogin');//执行收费设置操作
...
#会员管理
Route::get((empty(config('webset.webdir')) ? 'admin' : config('webset.webdir')) . '/userAdd','Admin\IndexController@userAdd')->middleware('CheckLogin');//添加会员
Route::get((empty(config('webset.webdir')) ? 'admin' : config('webset.webdir')) . '/userList/{name?}','Admin\IndexController@userList')->middleware('CheckLogin');//会员列表
Route::get((empty(config('webset.webdir')) ? 'admin' : config('webset.webdir')) . '/userEdit/{id}','Admin\IndexController@userEdit')->middleware('CheckLogin');//编辑会员
Route::get((empty(config('webset.webdir')) ? 'admin' : config('webset.webdir')) . '/kamiAdd','Admin\IndexController@kamiAdd')->middleware('CheckLogin');//生成卡密
Route::get((empty(config('webset.webdir')) ? 'admin' : config('webset.webdir')) . '/kamiList','Admin\IndexController@kamiList')->middleware('CheckLogin');//卡密列表
Route::get((empty(config('webset.webdir')) ? 'admin' : config('webset.webdir')) . '/kamiUrl','Admin\IndexController@kamiUrl')->middleware('CheckLogin');//卡密购买链接
Route::post('/action/userAdd', 'Admin\actionController@userAdd');//执行添加会员
Route::post('/action/userEdit', 'Admin\actionController@userEdit');//执行编辑会员
Route::post('/action/userDel', 'Admin\actionController@userDel');//执行删除会员
Route::post('/action/kamiAdd', 'Admin\actionController@kamiAdd');//执行生成卡密
Route::post('/action/kamiDel', 'Admin\actionController@kamiDel');//执行删除卡密
Route::post('/action/kamiUrl', 'Admin\actionController@kamiUrl');//执行卡密购买链接设置
Route::post('/action/kamiDelAllUse', 'Admin\actionController@kamiDelAllUse');//执行删除所有已用卡密
Route::get('getNotUse', function(){
    $file = date('Ymd-His').'.txt';
    $newFile = DATA_PATH.$file;
    $common = new App\Http\Controllers\Common\CommonController();
    $list =  $common->readData('kami');
    foreach ($list as $key => $row) {
        if ($row['status'] == 0) {
            file_put_contents($newFile,$row['kami']."\r\n",FILE_APPEND);
        }
    }
    return response()->download($newFile,$file);
});//提取未使用卡密
...
Route::get('admin', 'Admin\IndexController@index');//->middleware('CopyVerify','CheckLogin');//后台首页

Route::get('admin/video/{skey?}', 'Admin\IndexController@video');

//Route::get( 'admin' . '/userList/{name?}','Admin\IndexController@userList');//会员列表
